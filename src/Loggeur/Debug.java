...
public class Debug implements Logg{
	...
	/**
	 * Initialisation des dates de debug
	 */
	private Debug(){
		this.date = new Date();
	...
	public String afficherError(String message, int niveau, String chemin,String Consl,String File) throws IOException {
		...
		String messageFinal="";
		if(niveau<=1){
		 messageFinal = "["+this.dateFormat.format(this.date)+"]" + " (class Debug) : <" + message + ">\n";
	...
	public void afficherError(String message, int niveau, String[] Data) throws IOException {
		// TODO Auto-generated method stub
		if(niveau<=1){
		String messageFinal = "["+this.dateFormat.format(this.date)+"]" + " (class Debug) : <" + message + ">\n";
